...
> 创建日期: 2026-05-09 | 版本: v2.0 | 状态: 待实施
...
**验收标准**:
- [ ] 每条条目输出 > 500 字正文（之前50-200字）
- [ ] LLM 提取成功率 > 80%
- [ ] 无数据丢失（摘要至少保留为fallback）
- [ ] 6个RSS源全部通过
...
**验收标准**:
- [ ] JS 渲染页面正常获取
- [ ] 中文站点提取质量提升 > 50%（vs 纯 trafilatura）
- [ ] Scrapling 降级链命中率 > 30%（减少 LLM 调用）
- [ ] 新浪新闻首页等失败URL能正确提取
...
| 4.2 新建 `tools/fetchers/crawl4ai_adapter.py` | 封装Crawl4AI，保持llm_extractor.py相同接口 |
...
| 文件 | 原因 |
|---|---|
| `tools/pdf2md.py` | 独立工具，由arxiv_fetcher调用 |
...
| Token成本超预期 | 低 | 预算超标 | rate_limit硬限制; Ollama备选 | 月度成本报表 |
