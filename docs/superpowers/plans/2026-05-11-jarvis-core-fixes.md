...
Remove the old `cleaned = raw.strip()` → `parsed = json.loads(cleaned)` block.
...
Remove the old `start = raw.index("{")` → `data = json.loads(...)` block.
...
Expected: FAIL — `plan.estimated_cost` is None because loop never calls planner.
...
Expected: FAIL — `loop.learner` is None because learner is not initialized.
...
Expected: FAIL — `AttributeError: 'MultiAgentManager' object has no attribute 'start_task'`.
...
**1. Spec coverage:**
- ✅ Planner connected to loop (Task 2)
- ✅ Learner connected to loop (Task 3)
- ✅ Approval YAML parsing fixed (Task 4)
- ✅ `never_auto_approve` enforced (Task 4)
- ✅ MultiAgent task lifecycle added (Task 5)
- ✅ Shared utilities extracted (Task 1)
- ✅ Config loading unified (Task 6)
- ✅ Tests for all changes (every task has tests)

**2. Placeholder scan:**
- ✅ No "TBD", "TODO", "implement later"
- ✅ Every step shows exact code
- ✅ Every step shows exact commands and expected output
- ✅ No "Similar to Task N" shortcuts

**3. Type consistency:**
- ✅ `parse_llm_json` returns `list | dict | None` consistently across all call sites
- ✅ `load_yaml_config` signature matches all usage sites
- ✅ `ApprovalManager._never_auto` is `list[str]` throughout
- ✅ MultiAgent lifecycle methods return `bool` consistently
