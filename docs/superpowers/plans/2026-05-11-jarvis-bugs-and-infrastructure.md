...
**Architecture:** Patch existing `tools/jarvis/` modules in place. Extract cross-cutting concerns (config loading, state persistence) into dedicated modules. All changes are additive or bug-fixes — no interface breaking changes to `loop.py` public API.
...
Expected: FAIL — second poll returns 1 event instead of 0
...
Expected: FAIL — `SafetyEngine` has no `_daily_cost_usd` attribute
...
Expected: FAIL — `AgentLoop` has no `_consecutive_failures` attribute
...
1. **Spec coverage:** Every bug identified in the assessment has a task:
   - list_all() missing → Task 1 ✅
   - EventBus duplication → Task 2 ✅
   - SafetyEngine no budget → Task 3 ✅
   - Loop no auto-stop → Task 4 ✅
   - Missing config.py → Task 5 ✅
   - Missing state.py → Task 6 ✅
   - Empty __init__.py → Task 7 ✅

2. **Placeholder scan:** No TBD, TODO, or vague steps. Every step has actual code.

3. **Type consistency:**
   - `AgentLoop._consecutive_failures` used in init and start ✅
   - `SafetyEngine._check_budget()` used in pre_check ✅
   - `EventBus.mark_consumed()` used in loop ✅
