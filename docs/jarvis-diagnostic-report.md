...
| 3  | `/api/agent/chat` SSE 端点？ | ❌  | api\_server.py 中不存在此端点                        |
...
| 5.3 执行轨迹存储      | 将每次 AgentLoop 的完整执行轨迹（plan/tool\_calls/reflection/result）持久化到 PostgreSQL | Hermes Agent (trajectory\_compressor) | `tools/jarvis/loop.py` + DB schema                           |
...
| 5.6 工作流可视化      | 将执行轨迹渲染为 DAG 流程图，展示每步的状态和依赖                                              | EvoAgentX (workflow\_graph.display)   | 前端 `ExecutionDAG.tsx`                                        |
